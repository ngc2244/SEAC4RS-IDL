...
;	 Based on Jenny's get_field_data_arctas.pro
;
; CATEGORY:
;
; CALLING SEQUENCE:
...
;        Platform    - Name of Aircraft. Current options: 'DC8', 'ER2', 'WP3D', '*'
...
;
;        NoCities is not used currently
;        NoCities    - If set, the returned Data will not include observations
;                      near cities, eg., Fairbanks, Barrow, or Prudhoe Bay during ARCTAS.
;                      The criteria are currently <1.5 degrees from the site and <2 km altitude.
;
...
;	 lei, 29 Jul 2013: Updatd for SEAC4RS
;-
...
function get_field_data_seac4rs, Field_in, Platforms_in, FlightDates_in, $
  ...
  If Keyword_Set( minavg ) then avgtime='10s'
 ...
 ;--------------------------------------------------------------
  ...
  For i = 0, N_Entries-1L do begin
    ...
    If StrMatch( Platform[i], 'DC8', /fold_case ) then begin
      ...
      NewFiles = $
        MFindFile(!SEAC4RS+'/field_data/WP3D/'+mrgDir+$
                  '/*'+FlightDates[i]+'*.sav')
