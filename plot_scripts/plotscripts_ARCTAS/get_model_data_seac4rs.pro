...
;
;	 Based on Jenny's get_model_arctas.pro
; CATEGORY:
;
; CALLING SEQUENCE:
...
;
; KEYWORD PARAMETERS: (NOT use now)
;        NoCities    - If set, the returned Data will not include observations
;                      near cities, eg, Fairbanks, Barrow, or Prudhoe Bay in ARCTAS
;		       The criteria are currently <1.5 degrees from the site and <2 km
;                      altitude.
...
;        directory  e.g. ~/3SEAC4RS/quicklook 
;
; EXAMPLE:
;
...
function get_model_data_arctas, Field_in, Platforms_in, FlightDates_in,    $
  ...
  ; Number of platforms (e.g. DC8, ER2, WP3-D) and flight dates
  ...
  For i = 0, N_Entries-1L do begin
    ...
    endif else if StrMatch( Platform[i], 'WP3D', /fold_case ) then begin
