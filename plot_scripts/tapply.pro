...
function tapply, array, group, functionstr, groupvalues=groupvalues, $
   ...
   ; kludge for percentiles
   if ( functionstr eq 'percentiles' ) then $
      result = make_array( n_elements( groupvalues ),  n_elements( value ), type=type )
   ...
   for i=0, n_elements( groupvalues ) - 1L do begin
      ...
      ; kludge for percentiles
      if ( functionstr eq 'percentiles' ) then $
         result[i,*] = call_function( functionstr, array[index], value=value,_EXTRA=_EXTRA) $
      else $
